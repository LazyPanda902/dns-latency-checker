...
`dns-latency-checker` uses a small `src/` package with two layers:
...
The source reference `mayswind/ezbookkeeping` influenced only the problem-space research. Implementation details are intentionally independent.
